...
1. Define permission in `role-permissions.ts` (type → `ALL_PERMISSIONS` → `ROLE_PERMISSIONS`)
2. Apply to route (`canActivate` + `data.permissions`)
3. Apply to UI (`*appPermission`)
4. Apply to API endpoint (`requireManagement` / `requireAdmin`)
