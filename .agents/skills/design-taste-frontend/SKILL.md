...
> **Project adaptation (this repo only):** target stack is Angular (standalone components) + SCSS, not React/Next.js/Tailwind. Section 3 (Stack & Conventions) below is rewritten for this project. Everywhere else in this document:
> - Tailwind utility classes (`py-32`, `max-w-5xl`, `grid-flow-dense`, `rounded-[2rem]`, etc.) are **value specs**, not literal code — translate directly to the equivalent SCSS property (spacing/size/radius stay numerically identical).
> - React-only APIs (`useState`, `useMotionValue`, `motion/react`, `'use client'`, Server/Client Component split, `next/image`, `next/font`) have no Angular equivalent by name; the underlying intent (isolate heavy client-side interactivity, avoid unnecessary re-renders, optimize images/fonts) carries over via Angular signals, `ngAfterViewInit`/`ViewChild`, `NgOptimizedImage`, and standard `@font-face`.
> - Code blocks (GSAP/Motion skeletons in Section 5) are kept as React reference implementations with a short Angular-adaptation note attached — the design/motion logic they encode is unchanged.
> - The design rules, numeric values, and anti-pattern bans themselves are unchanged.
...
* **Framework:** Angular (standalone components).
  * **INTERACTIVITY ISOLATION:** Any component using GSAP, scroll listeners, or pointer physics MUST be an isolated component that sets up in `ngAfterViewInit` and tears down in `ngOnDestroy` - never run animation setup in the constructor or a template binding.
* **Styling:** **SCSS**, one file per component (Angular's default). No Tailwind.
* **Animation:** **GSAP** (+ `ScrollTrigger` where needed) called directly from the component class, or `@angular/animations` for simpler state-driven transitions.
* **Fonts:** Self-host with `@font-face` + `font-display: swap` in global styles. Never link Google Fonts via `<link>` in production.

### 3.B State
* Local component state: plain class fields, or Angular `signal()` for anything the template reads reactively.
* Shared state ONLY for deep prop-drilling avoidance - an injectable service holding signals or a small RxJS `BehaviorSubject`.
* **NEVER** drive continuous values from user input (mouse position, scroll progress, pointer physics, magnetic hover) through change detection / `signal.set()` on every frame. Mutate the DOM/style directly inside a `requestAnimationFrame` or GSAP tween, outside Angular's zone (`NgZone.runOutsideAngular`). Routing every mousemove through change detection re-renders the tree and collapses on mobile.

### 3.C Icons
* **Allowed:** any consistent SVG icon set with an Angular-friendly distribution (inline SVG components, Iconify, or a self-hosted sprite) - e.g. Phosphor, HugeIcons, Radix Icons, Tabler, sourced as raw SVG rather than a React component package.
* **Discouraged:** Lucide as the default choice. Acceptable only when the user explicitly asks for it or the project already depends on it.
* **NEVER hand-roll SVG icons.** If a glyph is missing, pull a second set or compose from primitives - do not draw icon paths from scratch.
...
*Angular adaptation: move the `gsap.context` setup into `ngAfterViewInit`, call `ctx.revert()` in `ngOnDestroy`, and read `prefers-reduced-motion` via `window.matchMedia` instead of `useReducedMotion()`. `ref` becomes a `@ViewChild` element ref. The GSAP/ScrollTrigger calls and their config values are unchanged.*
...
*Angular adaptation: same as 5.A - `ngAfterViewInit`/`ngOnDestroy` lifecycle, `@ViewChild` refs instead of `useRef`. GSAP tween/ScrollTrigger config is unchanged.*
...
For simple "items appear as they enter viewport" (no pinning), prefer a plain `IntersectionObserver` set up in `ngAfterViewInit` over GSAP - lighter, no ScrollTrigger needed. React reference below; the trigger/threshold/stagger logic maps directly onto the observer callback:
...
* **Custom scroll progress calculations using `window.scrollY`** driven through Angular signals/change detection. Same reason. Re-renders on every frame.
* **`requestAnimationFrame` loops that touch Angular signals or trigger change detection.** Mutate styles directly, run outside `NgZone`, instead.
* **Layout Transitions:** Use GSAP's `flip` plugin or `@angular/animations` for visible state changes (re-ordering lists, expanding modals, shared elements between routes). Do not wrap static content in transition logic "for safety" - it costs measurement work.
* **Staggered Orchestration:** Use GSAP's `stagger` option or CSS cascade (`animation-delay: calc(var(--index) * 100ms)`) for reveal moments where sequence matters. Parent and staggered children must be set up from the same component's lifecycle hook.
...
* **LCP** < 2.5s. Hero image must use Angular's `NgOptimizedImage` with `priority` or be preloaded via a `<link rel="preload">`.
...
* **`@angular/animations`** - default for UI / Bento / state-change motion.
* **GSAP + ScrollTrigger** - for full-page scrolltelling and scroll hijacks. Isolate in dedicated components with `ngAfterViewInit`/`ngOnDestroy` cleanup.
* **Three.js / WebGL** - for canvas backgrounds and 3D scenes. Same isolation rule.
* **NEVER mix GSAP / Three.js with `@angular/animations` driving the same elements.** They fight over the same frames.
...
3. **Code sketch** - minimal working implementation (static Angular component by default, motion logic isolated in its own component with `ngAfterViewInit`/`ngOnDestroy`).
