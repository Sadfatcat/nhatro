...
> **Project adaptation (this repo only):** target stack is Angular + SCSS, not React/Tailwind. Wherever this file says `className`, use Angular's `class`/`[ngClass]`. Wherever it says `@gsap/react`, use plain `gsap` + `ScrollTrigger` called from `ngAfterViewInit`/`ngOnDestroy` (no React hooks exist in Angular). Tailwind utility classes below (`py-32`, `max-w-5xl`, `grid-flow-dense`, etc.) are value specs, not literal code — translate each to the equivalent SCSS property (e.g. `py-32` → `padding-block: 8rem`, `max-w-5xl` → `max-width: 64rem`, `grid-flow-dense` → `grid-auto-flow: dense`). The design rules and numeric values themselves are unchanged.
...
Static interfaces are strictly forbidden. You must write real GSAP (`gsap`, `ScrollTrigger`).
...
- **Inline Typography Images:** Embed small, pill-shaped images directly INSIDE massive headings. Example (Angular template): `I shape <span class="inline-pill-image" [style.background-image]="'url(...)'"></span> digital spaces.` (`.inline-pill-image` = `display: inline-block; width: 6rem; height: 2.5rem; border-radius: 9999px; vertical-align: middle; background-size: cover; background-position: center; margin-inline: 0.5rem;`)
